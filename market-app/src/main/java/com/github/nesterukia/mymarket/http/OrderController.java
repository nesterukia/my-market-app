...
public class OrderController {
    ...
    @Autowired
    public OrderController(OrderService orderService, CartService cartService, UserService userService, ItemService itemService) {
    ...
    public Mono<String> buy(@CookieValue(value = USER_ID_COOKIE, required = false) Long userId, ServerWebExchange exchange) {
        ...
        return userService.getOrCreate(userId, exchange)
                ...
                .flatMap(cart -> orderService.createOrderFromCart(cart).flatMap(
                        newOrder -> cartService.clearCartAndDelete(cart).thenReturn(newOrder.getId())
                        )
                )
                .map(redirectTemplate::formatted);
    }
}
